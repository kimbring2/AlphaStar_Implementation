...
writer = tf.summary.create_file_writer(workspace_path + "/tensorboard/reinforcement_learning/")
...
def train_reinforcement_step(
  ...
  with tf.GradientTape() as tape:
    ...
    prediction = run_reinforcement_episode(initial_feature_screen, initial_feature_minimap, initial_player, initial_feature_units, 
    			      initial_game_loop, initial_available_actions, initial_build_queue, initial_single_select, initial_multi_select,
                initial_score_cumulative, initial_memory_state, initial_carry_state, initial_done, model, max_steps_per_episode) 
    ...
    loss = compute_reinforcement_loss(fn_probs, screen_arg_probs, minimap_arg_probs, screen2_arg_probs,
  ...
  return (episode_reward, feature_screen, feature_minimap, player, feature_units, game_loop, available_actions, 
            build_queue, single_select, multi_select, score_cumulative, memory_state, carry_state, done)
...
with tqdm.trange(max_episodes) as t:
  for i in t:
    ...
    train_result = train_reinforcement_step(initial_feature_screen, initial_feature_minimap, initial_player, 
    ...
    if i % 1000 == 0:
      model.save_weights(workspace_path + "/Models/Reinforcement_Learning" + env_name + "_Model")
