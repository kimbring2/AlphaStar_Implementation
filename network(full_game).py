...
class AlphaStar(tf.keras.Model):
  ...
  def call(self, feature_screen, feature_minimap, player, feature_units, memory_state, carry_state, game_loop,
    ...
    core_output, memory_state, carry_state = self.core(feature_encoded, memory_state, carry_state)
    ...
    for arg_type in actions.TYPES:
      if arg_type.name == 'screen':
        ...
        self.args_out_logits[arg_type] = self.minimap_argument_head(feature_minimap_encoded, core_output, autoregressive_embedding)
...
class FullyConv(tf.keras.Model):
  ...
  def call(self, feature_screen, feature_player, feature_units,  memory_state, carry_state, game_loop, available_action):
    batch_size = tf.shape(feature_screen)[0]
...
def make_model(name):
    if name == 'fullyconv':
      ...
      fn_out, args_out, value, final_memory_state, final_carry_state = FullyConv(32, 32)(feature_screen, player, feature_units, 
                                                                                                         memory_state, carry_state,
                                                                                                         game_loop, available_actions)
      model = tf.keras.Model(inputs={'feature_screen': feature_screen, 'player': player, 'feature_units': feature_units, 
