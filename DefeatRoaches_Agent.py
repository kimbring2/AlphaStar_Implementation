...
class DefeatRoaches(base_agent.BaseAgent):
 ...
 274/HoldPosition_quick                                 (3/queued [2])
  ...
  def reset(self):
    ...
    self._marine_selected = True
    self._marine_action_ended = True
    self._marine_group_selected = False
    self._marine_group_action_ended = False
    self.step_num = 1
  ...
  def step(self, obs):
    ...
    if (self.step_num % 5 == 0):
      ...
      marine_x_mean = statistics.mean(marine_x_list)
      roache_x_mean = statistics.mean(roache_x_list)
      marine_y_mean = statistics.mean(marine_y_list)
      roache_y_mean = statistics.mean(roache_y_list)

      marine_unit = marines[0]
      marine_xy = [marine_unit.x, marine_unit.y]
      ...
      if ( (self._marine_selected == False) & (self._marine_group_action_ended == True) | (len(marines) == 1) ):
        if not marine_unit.is_selected:
        # Nothing selected or the wrong marine is selected.
          self._marine_selected = True
          return FUNCTIONS.select_point("select", marine_xy)
      
      elif ( (self._marine_group_selected == False) & (self._marine_action_ended == True) & (len(marines) > 1) ):
        self._marine_group_selected = True
      ...
      if ( (self._marine_selected == True) & (self._marine_action_ended == False) ):
        if FUNCTIONS.Attack_screen.id in obs.observation.available_actions:
          ...
          return FUNCTIONS.no_op()
      elif ( (self._marine_group_selected == True) & (self._marine_group_action_ended == True) & (len(marines) > 1) ):
        if FUNCTIONS.Attack_screen.id in obs.observation.available_actions:
          if ( (remain_marines_x_mean > roache_x_mean) & (remain_marines_y_mean > roache_y_mean) ):
            x_point = roache_x_mean + 25
            y_point = roache_y_mean - 25
          elif ( (remain_marines_x_mean > roache_x_mean) & (remain_marines_y_mean < roache_y_mean) ):
            x_point = roache_x_mean + 25
            y_point = roache_y_mean + 25
          elif ( (remain_marines_x_mean < roache_x_mean) & (remain_marines_y_mean > roache_y_mean) ):
            x_point = roache_x_mean - 25
            ...
            x_point = remain_marines_x_mean
            y_point = remain_marines_y_mean

          self._marine_group_action_ended = True
