...
COMMAND_LEARNER='python learner.py --env_num '"${NUM_ACTORS}"' --gpu_use '"${GPU_USE}"' --model_name '"${MODEL_NAME}"'  --gradient_clipping '"${GRADIENT_CLIPPING}"''
...
for ((id=0; id < $NUM_ACTORS; id++)); do
    ...
    COMMAND='python actor.py --env_id '"${id}"' --environment '"${ENVIRONMNET}"''
